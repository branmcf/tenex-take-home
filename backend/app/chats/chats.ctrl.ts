...
import { createChatSSEHelpers } from './chats.helper';
import {
    GetUserChatsRequest
...
import { CHAT_EVENTS_TIMEOUT_MS } from '../../utils/constants';
...
export const streamChatEventsHandler = async (
    ...
    // create SSE helpers with cleanup callback
    const sseHelpers = createChatSSEHelpers( res, () => {
        if ( timeoutId ) clearTimeout( timeoutId );
        if ( unsubscribe ) unsubscribe();
    } );

    // send initial connected event
    sseHelpers.sendEvent( { type: 'connected', chatId } );

    // subscribe to title updates for this chat
    unsubscribe = chatEvents.onTitleUpdated( chatId, ( data ) => {
        sseHelpers.sendEvent( {
            type: 'title_updated'
            , chatId: data.chatId
            , title: data.title
        } );
    ...
    timeoutId = setTimeout( () => {
        if ( !sseHelpers.isClosed() ) {
            sseHelpers.sendEvent( { type: 'timeout' } );
            sseHelpers.cleanup();
        }
    }, CHAT_EVENTS_TIMEOUT_MS );

    // handle client disconnect
    req.on( 'close', sseHelpers.cleanup );
